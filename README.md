Tela de Login:
Usada para o cadastro de novos usuários no formulário.
<img width="1912" height="938" alt="image" src="https://github.com/user-attachments/assets/5a098e3d-2fcf-4c1e-83f7-46e47004cc2e" />

Painel:
Este dashboard apresenta um resumo claro dos dados dos alunos. Os cards mostram informações rápidas, como total de alunos, média de idade, bairro com mais cadastros e tipos de responsáveis. Os gráficos exibem a distribuição de alunos por curso e por bairro, facilitando a visualização. Também há listagens como alunos mais velhos, mais novos, CEPs com mais registros e responsáveis com mais de um aluno, oferecendo uma visão completa do perfil dos cadastrados.

<img width="1339" height="649" alt="image" src="https://github.com/user-attachments/assets/6eeea409-d5a7-465e-9798-2987231afa61" />
<img width="1908" height="938" alt="image" src="https://github.com/user-attachments/assets/4ea39803-b442-458d-89ed-12c38f65eacd" />

Tela de Registro:
Esta página é destinada ao registro de novos usuários, permitindo inserir todas as informações necessárias para incluí-los no sistema.
<img width="1915" height="933" alt="image" src="https://github.com/user-attachments/assets/130fd60e-512d-4625-84b8-34f069499c59" />

Tela de Cadastro 
Esta página é destinada ao registro de novos alunos, permitindo inserir todas as informações necessárias para incluí-los no sistema.
<img width="1883" height="934" alt="image" src="https://github.com/user-attachments/assets/64fab7b1-e246-4df5-8010-45b15a0baf52" />




